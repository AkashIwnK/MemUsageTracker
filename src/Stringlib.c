...
/* We make the current printf buffer pointer */
	...
	while(1) {
		...
		/* Check what the user is planning to print. We have to do it ourselves
		 * because sprintf may not a reiable function in this case.
		 */
			string_end += 1;
			if(*string_end == 'c') {
				...
				*((char *)cur_printfPtr++) = (char)int_form_char;
				...
				/* Get the integer and put it in the buffer one by one in the buffer */
					...
					if(*string_end == 's') {
						...
						if(*string_end == 'p') {
						/* Here we start storing the hexadecimal string in the reverse
						 * order of computation in the bottom of the buffer first.
						 */
						...
						save_string:
						/* We keep track of the end of the string that is going to be
						...
						/* If quotient is negative, we just make sure we convert it to a
						 * positive number.
						 */
						...
						/* We put the colon after the negative hexaddecimal number and a
						 * dash to indicate that it is a negative number.
						 */
								*((char *)cur_printfPtr++) = ':';
								*((char *)cur_printfPtr++) = ' ';
								*((char *)cur_printfPtr++) = '-';
...
/* We do not depend on the string library because apprently some functions
* can be unreliable.
*/
				...
				*((char *)cur_printfPtr++) = '0' + int_form_char;
				...
				/* Get the integer and put it in the buffer one by one in the buffer */
					...
					if(*string_end == 's') {
						...
						/* We are not done yet. We need to store the string to the rest
						 * of the string written near the top of the buffer.
						 */
							temBufPtr--;
							while(temBufPtr != string_end) {
								*((char *)cur_printfPtr++) = *((char *)(temBufPtr--));
...
/* Now,that we have checked if the format of the string is correct, we proceed
 * to compute the numerical value of the hex in the string from the end, all
 * the way up to the beginning of it.
 */
	uint64_t value = 0;
	uint64_t numBytes = 0;
	while(stringEnd >= stringStart) {
		...
		value += fourByteVal*((uint64_t)1 << numBytes); /* Scale for the number of
																										 * bytes traversed */
		stringEnd--;
...
/* We iterate through the string and see if the number is potentially a
 * negative number.
 */
	int8_t numIsNeg = 0;
	char *charPtr = string;
...
/* Now we scan through the rest of the string to make sure that it conatains
 * valid numbers. At the same time we count the number of digits.
 */
	char *ptr = charPtr;
	uint64_t count = 0;
	while(*ptr) {
